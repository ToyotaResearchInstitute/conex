...
TEST(TestArguments, AddLMI) {
  ...
  void* null_ptr = NULL;
  EXPECT_TRUE(CONEX_NewLorentzConeConstraint(null_ptr,  2, &constraint_id) == CONEX_FAILURE);

  void* corrupted_ptr = static_cast<double*>(p) + 1;
  EXPECT_TRUE(CONEX_NewLorentzConeConstraint(corrupted_ptr,  2, &constraint_id) == CONEX_FAILURE);
...
TEST(TestArguments, UpdateLMI) {
  ...
  status = CONEX_UpdateLinearOperator(p, constraint_id, .3, 2, order - 1, 0, bad_hyper_complex_dim);
  EXPECT_EQ(CONEX_FAILURE, status); 
  status = CONEX_UpdateLinearOperator(p, constraint_id, .3, 2, bad_variable, 0, 0);
  EXPECT_EQ(CONEX_FAILURE, status); 
  status = CONEX_UpdateLinearOperator(p, constraint_id, .3, 2, order - 1, bad_column_index, 0);
  EXPECT_EQ(CONEX_FAILURE, status); 
  ...
  status = CONEX_UpdateAffineTerm(p, constraint_id, .3, 2, 0, bad_hyper_complex_dim);
  EXPECT_EQ(CONEX_FAILURE, status); 
