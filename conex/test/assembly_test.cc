...
std::vector<int> ReplaceWithPosition(
...
std::vector<int> ConcatFirstN(const std::vector<int>& a, int N, const std::vector<int>& b) {
  std::vector<int> y;
...
void BuildLQRProblem(int N, ConstraintManager *prg) {
  ...
  int o = 0;
  for (int i = 0; i < N; i++) {
    vector vars{1 + o, 2 + o, 3 + o, 4 + o, 5 + o}; o += 3;
...
TEST(LDLT, TestAssembly) {
  ...
  Pt.indices() = Eigen::Map<Eigen::MatrixXi>(data.permutation_inverse.data(), data.N, 1);
  ...
  MatrixXd G = TriangularMatrixOperations::ToDense(mat).selfadjointView<Eigen::Lower>(); MatrixXd error = (Pt * G * Pt.transpose()  - T);
...
TEST(LDLT, Benchmark2) {
  ...
  VectorXd b(data.N); b.setConstant(1);
  VectorXd y = b;
  TIME(std::vector<Eigen::LDLT<Eigen::Ref<MatrixXd>>> factorization;
  BlockTriangularOperations::BlockLDLTInPlace(&mat.workspace_, &factorization);
  BlockTriangularOperations::SolveInPlaceLDLT(mat.workspace_, factorization, &y););
