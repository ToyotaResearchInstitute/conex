...
TEST(Eigenvalues, TruncatedApproximiationInterlaces) {
  ...
  int n_approx = 2;
  VectorXd eigJ = ApproximateEigenvalues(A, MatrixXd::Identity(n, n), r0, n_approx, true); 
...
TEST(Eigenvalues, Lanczos) {
  ...
  VectorXd r0(n);
  r0 << 1, 2, 0, 4;
  VectorXd eigJ = ApproximateEigenvalues(A, MatrixXd::Identity(n, n), r0, n, true); 
  auto eigA = eig(A).eigenvalues;
