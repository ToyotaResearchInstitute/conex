...
class LowerTriangularSuperNodal {
 ...
 public:
  ...
  void Init() {
    ...
    //   separator_.at(j).resize(supernode_size_.at(j),  path_.at(j).size() - supernode_size_.at(j));
    // }
  }
...
Eigen::MatrixXd TriangularMatrixOperations::ToDense(const SparseTriangularMatrix& mat) {
...
SparseTriangularMatrix MakeSparseTriangularMatrix(int N, const std::vector<Clique>& path_) {
  auto path = path_;
  Sort(&path);
