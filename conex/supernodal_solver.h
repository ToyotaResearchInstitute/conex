...
inline void Bind(MatrixData& data, 
          TriangularMatrixWorkspace& workspace,
  ...
  for (int e = static_cast<int>(eqs.size()) - 1; e >=0; e--) {
    ...
    auto block = BuildBlock(&sn.at(e), &sep.at(e), workspace.off_diagonal.at(e).data());
    ...
    if (workspace.seperator_diagonal.at(e).size() > 0) {
      block = BuildBlock(&sep.at(e), &sep.at(e), &workspace.seperator_diagonal.at(e));
      eqs.at(i).BindOffDiagonalBlock(&block);
    }
  }
}
...
struct SparseTriangularMatrix {
  ...
  SparseTriangularMatrix(int N_,
                         const std::vector<Clique>& cliques, 
                         const std::vector<int>& supernode_size) :
                         SparseTriangularMatrix(N_, cliques, supernode_size, 
                                        Eigen::VectorXd::Zero(
                                        SizeOf(TriangularMatrixWorkspace(cliques, supernode_size)))
                                        ) { }

  SparseTriangularMatrix(MatrixData& data) : SparseTriangularMatrix(data.N, data.cliques, 
                                                                    data.supernode_size) {}
