...
template<typename T>
bool UpdateLinearOperator(T* obj, double val, int var, int row, int col, int hyper_complex_dim) {
  CONEX_DEMAND(false, "Constraint does not support updates of linear operator.");
}
...
class Constraint {
 public:
  ...
  friend bool UpdateLinearOperator(Constraint* o, double val, int var, int row, 
    ...
    return o->model->do_update_linear_operator(val, var, row, col, hyper_complex_dim);
  }

  friend bool UpdateAffineTerm(Constraint* o, double val, int row, 
                                   int col, int hyper_complex_dim) { 
 ...
 private:
  struct Concept {
    ...
    virtual bool do_update_linear_operator(double val, int var, 
                                           int row, int col, int hyper_complex_dim) = 0;
    virtual bool do_update_affine_term(double val, int row, int col, int hyper_complex_dim) = 0;
  ...
  struct Model final : Concept {
    ...
    bool do_update_linear_operator(double val, int var, int row, int col, int hyper_complex_dim) override {
       return UpdateLinearOperator(&data, val, var, row, col, hyper_complex_dim);
    }

    bool do_update_affine_term(double val, int row, int col, int hyper_complex_dim) override {
