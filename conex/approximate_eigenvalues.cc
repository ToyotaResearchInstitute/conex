...
//    
//    Example: for the inner-product  <p, q> = <p(A) r_0, p(A) r_0> and symmetric A, reduces to
//    Lanczos method where we use the simplification
//
//                        <q_i(A) r_0, A r_0> = 
//
...
class JacobiSolver {
 public:
  JacobiSolver(const MatrixXd& A, const MatrixXd& W, 
               const VectorXd& r0, int n) : W_(W), r0_(r0), n_(n), powers_of_A_(n + 1) {
...
Eigen::VectorXd EigenvaluesOfJacobiMatrix(const Eigen::MatrixXd& A,const Eigen::MatrixXd& W, const Eigen::VectorXd& r0,  int n) {
